...
const ChatWindow: React.FC<ChatWindowProps> = ({ session, messages, currentUser, users, onSendMessage }) => {
  ...
  const renderAttachment = (attachment: Attachment) => {
    ...
    return (
      <div className="mt-2 flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold truncate dark:text-gray-200">{attachment.name}</p>
          <p className="text-[10px] text-gray-500">{(attachment.size / 1024).toFixed(1)} KB</p>
        </div>
        <a 
          href={attachment.url} 
          download={attachment.name}
          className="p-2 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-full text-gray-400 hover:text-primary transition-colors"
        >
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900">
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center opacity-30 select-none pointer-events-none text-center">
            ...
            <p className="text-sm font-medium text-gray-400">No messages yet. Start the conversation!</p>
          </div>
        )}
        {messages.map((msg) => {
            ...
            <div key={msg.id} className={`flex items-end gap-2 ${isMeSent ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className={`flex flex-col max-w-[70%] ${isMeSent ? 'items-end' : 'items-start'}`}>
                {session.type === 'group' && !isMeSent && (
                  <span className="text-[10px] font-bold text-gray-400 mb-1 ml-1">{sender.name}</span>
                )}
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-white/50 dark:bg-gray-900/50">
        <form onSubmit={handleSend} className="flex gap-2 items-center">
          ...
          <button
            ...
            className="bg-primary text-white p-2.5 rounded-xl hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md shadow-primary/20"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
